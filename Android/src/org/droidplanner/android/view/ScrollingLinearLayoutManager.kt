...
class ScrollingLinearLayoutManager(
    ...
    override fun smoothScrollToPosition(
        ...
        val itemSize =
            if (orientation == HORIZONTAL) firstVisibleChild.width else firstVisibleChild.height
        ...
        if (distanceInPixels == 0) {
            distanceInPixels =
                Math.abs(if (orientation == HORIZONTAL) firstVisibleChild.x else firstVisibleChild.y)
                    .toInt()
        }

        val smoothScroller =
            SmoothScroller(recyclerView.context, distanceInPixels.toFloat(), duration)
